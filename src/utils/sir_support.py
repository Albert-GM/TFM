...
def countries_reaction(t, react_time, top_countries):
    """
    Computes how long a country takes to react once the death limit is exceeded. 
...
def first_death(deaths_world_t, period=14):
    """
    Computes the first death that occurs in the pandemic and returns that day
    and the day for which period days have passed

    Parameters
    ----------
    deaths_world_t : np.array
    period : int, optional
    ...
    day_1 : float
        Day of first death.
    day_2 : TYPE
        Day of first death plus period.

    """

    day_1 = np.argwhere(deaths_world_t > 0)[0, 0]
    day_2 = day_1 + period
...
def mortality_power(deaths_world_t, new_infected_world_t, SIR_world_t,
                    day_1, day_2):
    """
    ...
    ratio_1 = deaths_world_t[:day_2].sum(
    ) / (new_infected_world_t[day_1:day_2].sum())

    ratio_2 = (deaths_world_t[day_1:day_2].sum() /
               (SIR_world_t[1, day_2] - SIR_world_t[1, day_1]))

    ratio_3 = (deaths_world_t[day_1:day_2].sum() /
               (SIR_world_t[2, day_2] - SIR_world_t[2, day_1]))

    gradient = np.gradient(deaths_world_t[day_1:day_2])
